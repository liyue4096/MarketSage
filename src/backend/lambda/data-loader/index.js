...
async function fetchAllTickersSnapshot(apiKey) {
...
async function fetchAggregates(apiKey, ticker, fromDate, toDate) {
...
async function fetchSMA(apiKey, ticker, timestamp, window) {
...
async function loadSnapshotData(pool, snapshots, tradeDate) {
...
const handler = async (event) => {
    ...
    try {
        ...
        if (action === 'load-snapshot' || action === 'full') {
            ...
            // Verify data
            const [metadataCount] = await pool.query('SELECT COUNT(*) as count FROM ticker_metadata').then(r => r.rows);
            const [priceCount] = await pool.query('SELECT COUNT(*) as count FROM price_history WHERE trade_date = $1', [tradeDate]).then(r => r.rows);
        ...
        if (action === 'query') {
            ...
            const result = await pool.query(`SELECT trade_date::text, ticker, o::float, h::float, l::float, c::float, v::float, vw::float,
        ...
        if (action === 'query-sma') {
            const tickersToQuery = event.tickers || ['AAPL', 'MSFT', 'GOOGL', 'AMZN', 'TSLA'];
            const result = await pool.query(`SELECT trade_date::text, ticker, sma_20::float, sma_60::float, sma_250::float, last_updated
         FROM sma
         WHERE ticker = ANY($1)
      ...
      `);
            ...
            try {
                ...
                for (const record of russellData) {
                    await client.query('INSERT INTO russell_1000 (ticker, name) VALUES ($1, $2)', [record.ticker, record.name]);
                    inserted++;
                }
                await client.query('COMMIT');
        ...
        if (action === 'load-russell-agg') {
            ...
            const russellResult = await pool.query('SELECT ticker FROM russell_1000 ORDER BY ticker');
